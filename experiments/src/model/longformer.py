...
class CartographyCallback(TrainerCallback):
    def __init__(self, trainer, dataset, dataset_name, seed):
        ...
        self.save_path = f"experiment_results/cartography/{dataset_name}_{seed}.tsv"
...
def train_longformer(X_train, y_train, X_val, y_val, X_test, y_test, model_save_path, logging_dir, dataset_name, logger, batch_size, accumulation_steps, classification_type="standard", weighted_training=False, seed=42):
    # Prepare datasets
    ...
    def model_init():
        return AutoModelForSequenceClassification.from_pretrained("allenai/longformer-base-4096", num_labels=len(set(y_train)), id2label=id2label, label2id=label2id)
    ...
    if classification_type == "relation/stance":
        ...
        def tokenize_function(examples):
            return tokenizer(examples['text'], examples['query'], padding="max_length", truncation=True, max_length=4096)
    else:
    ...
    # if dataset_name in ["climateFEVER_evidence", "climateStance", "lobbymap_pages", "climateFEVER_evidence_climabench", "netzero_reduction", "climaQA"]:
    ...
    #         disable_tqdm=True,
    #
    #         # Using default parameters for the following:
    #         # fp16=True,
    #         # metric_for_best_model="f1",
    #         # greater_is_better=True,
    #         # warmup_ratio=0.1,
    ...
    save_mappings(
        mapping_file="experiment_results/label_mappings.json",
        label_map=label2id,
        dataset_name=dataset_name
    )
...
def train_multi_longformer(X_train, y_train, X_val, y_val, X_test, y_test, model_save_path, logging_dir, dataset_name, logger, batch_size, accumulation_steps, classification_type="standard", weighted_training=False, seed=42):
    ...
    def model_init():
        return AutoModelForSequenceClassification.from_pretrained("allenai/longformer-base-4096", num_labels=n_labels, id2label=id2label, label2id=label2id, problem_type="multi_label_classification")
